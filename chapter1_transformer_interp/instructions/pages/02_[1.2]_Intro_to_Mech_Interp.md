...
> **Colab: [exercises](https://colab.research.google.com/github/callummcdougall/ARENA_3.0/blob/main/chapter1_transformer_interp/exercises/part2_intro_to_mech_interp/1.2_Intro_to_Mech_Interp_exercises.ipynb?t=20260520) | [solutions](https://colab.research.google.com/github/callummcdougall/ARENA_3.0/blob/main/chapter1_transformer_interp/exercises/part2_intro_to_mech_interp/1.2_Intro_to_Mech_Interp_solutions.ipynb?t=20260520)**
...
> **Notation bridge — what W_Q, W_K, W_V, W_O do:**
>
> Attention lets each token gather information from other tokens in three steps: *decide where to look* (Q/K), *gather what to take* (V), and *write it back* (O).
>
> In **classical QKV notation**, for one head at a single layer:
> - **Query**: `q = x @ W_Q` — each token asks "what am I looking for?" (shape: `[seq, d_head]`)
> - **Key**: `k = x @ W_K` — each token announces "what do I contain?" (shape: `[seq, d_head]`)
> - **Value**: `v = x @ W_V` — each token says "here is what I'll share" (shape: `[seq, d_head]`)
> - **Attention score**: `score[i, j] = (q[i] · k[j]) / √d_head` — how much token i looks at token j
> - **Output**: `out[i] = (softmax(score[i]) @ v) @ W_O` — weighted mix of values, projected back to `d_model`
>
> The **Mathematical Framework** (Anthropic, 2021) collapses the V and O steps into named circuits:
> - **W_OV = W_V W_O**: the combined "read-then-write" map. Instead of computing v and then multiplying by W_O separately, W_OV does both in one matrix: it maps from the source token's residual stream vector directly to what gets added to the destination.
> - **W_QK = W_Q (W_K)^T**: the bilinear form for attention scores. `score[i, j] = x[i] @ W_QK @ x[j].T` replaces the two-step `q = x @ W_Q; k = x @ W_K; score = q @ k.T`.
>
> This factored view makes the *function* of each head cleaner: the QK circuit decides *where* to look; the OV circuit decides *what to copy*. You'll see these names throughout Section 4.
>
> **Why separate `head_index` and `d_head` axes?** Each head runs its own independent Q/K/V subspace in parallel. Keeping the axes separate lets you select `model.W_Q[layer, head]` to get exactly one head's matrix — no reshaping arithmetic needed.
>
> **From the paper — formal definitions (Elhage et al., 2021 §"Attention Heads as Low-Rank Decompositions"):**
>
> The paper defines $W_{QK}^h$ and $W_{OV}^h$ as $d_{model} \times d_{model}$ matrices that collapse the per-head projections:
> $$W_{QK}^h := (W_Q^h)^T W_K^h, \qquad W_{OV}^h := W_O^h W_V^h \quad \text{(paper: column-vector convention)}$$
> In TransformerLens (row-vector convention) the weight shapes are transposed, giving the equivalent:
> $$W_{QK}^h := W_Q^h (W_K^h)^T, \qquad W_{OV}^h := W_V^h W_O^h \quad \text{(TransformerLens)}$$
> $W_{QK}$ is the **same matrix** in both conventions; $W_{OV}^{\text{TL}} = (W_{OV}^{\text{paper}})^T$.
>
> The **full circuits** connect these directly to the vocabulary:
> $$\text{Full OV circuit: } W_E \, W_{OV}^h \, W_U \in \mathbb{R}^{d_\text{vocab} \times d_\text{vocab}}$$
> Row $t$ gives the change in output logits caused when the destination attends to source token $t$.
> $$\text{Full QK circuit: } W_E \, W_{QK}^h \, W_E^T \in \mathbb{R}^{d_\text{vocab} \times d_\text{vocab}}$$
> Entry $(t_i, t_j)$ gives the unnormalized attention score from query token $t_i$ to key token $t_j$.
>
> For a **one-layer attention-only model**, the paper shows the logits at position $s$ decompose exactly as:
> $$\text{logits}[s] = x_s W_U + \sum_{h} \sum_{s'} A^h_{s,s'} \cdot x_{s'} W_{OV}^h W_U$$
> where $x_s = t_s W_E$ is the residual stream at position $s$ and $A^h_{s,s'}$ is the attention weight from $s$ to $s'$. The first term is the **direct path** (embedding straight to unembedding); each second term is one head's contribution. This additive decomposition is possible precisely because the residual stream is a sum of all component outputs.
...
* They develop fairly suddenly in a phase change - from about 2B to 4B tokens we go from no induction heads to pretty well developed ones. This is a striking divergence from a 1L model [see the comparison of in context learning performance curves for models with different layers](https://transformer-circuits.pub/2022/in-context-learning-and-induction-heads/index.html#:~:text=Our%20first%20observation) and can be observed in much larger models (eg a 13B one)
...
> **In plain language — the induction circuit step by step:**
>
> Suppose the input sequence is `[... A B ... A B ...]` and the model has just seen the second `A` and needs to predict `B`.
>
> **Step 1 — Previous-token head (layer 0, e.g. head 0.7):** Its QK circuit attends one position back (position `i` attends to position `i-1`). Its OV circuit copies the attended token's information. So at the position of the first `A`, this head writes: "the token *before* me is `B`" into the residual stream.
>
> **Step 2 — Induction head (layer 1, e.g. head 1.4), via K-composition:** This head's keys are computed not from the raw token embedding but from the *output of head 0.7*. So each position's key effectively encodes "what token came after the previous occurrence of my token?" When the second `A` computes its query and compares to all keys, it finds the strongest match at the position of the first `A` — where the key encodes `B` (the token that followed the first `A`). The induction head attends there and copies `B`-information forward as the next-token prediction.
>
> **Why this requires 2 layers:** A single-layer head can only attend based on token identity (who is at position i vs position j). It cannot attend based on "what token appears *next to* position j" — that requires reading information that another head already wrote to the residual stream. K-composition is the mechanism that enables this cross-layer coordination.
>
> **From the paper — K-composition circuit (Elhage et al., 2021 §"Two-Layer Attention-Only Transformers"):**
>
> The paper formalizes K-composition as a **virtual QK circuit** formed by composing two heads. When head $h_1$'s keys are partly determined by head $h_0$'s output (via the residual stream), the effective attention score at $h_1$ picks up an extra term:
> $$\text{K-comp contribution: } \; x_i \, W_{OV}^{h_0} \, W_{QK}^{h_1} \, x_j^T$$
> where $x_i$ is the query-position residual stream and $x_j$ is the key-position residual stream. Expanding through the embedding gives the **full K-composition circuit**:
> $$W_E \, W_{OV}^{h_0} \, W_{QK}^{h_1} \, W_E^T \;\in\; \mathbb{R}^{d_\text{vocab} \times d_\text{vocab}}$$
> Entry $(A, B)$ of this matrix is the contribution to the attention score paid to source token $A$, by any query position that strongly attended to token $B$ in head $h_0$.
>
> For the induction circuit ($h_0 = 0.7$, $h_1 = 1.4$): the large entries are at $(B, A)$ — meaning head 1.4 attends to $B$ when the query position previously saw $A \to B$ in head 0.7. This is exactly the "find the token after the previous occurrence" behavior.
>
> The paper also identifies a **positional** component: the prev-token head's QK circuit is dominated by $W_\text{pos} W_{QK}^{0.7} W_\text{pos}^T$ (position $i$ attends to $i-1$), and this propagates into 1.4's keys via $W_\text{pos} \, W_{OV}^{0.7} \, W_{QK}^{1.4} \, W_\text{pos}^T$. Together, token-level and position-level K-composition terms implement the full induction pattern.
...
<details><summary>Click to see the expected output (yours might be slightly different due to randomness)</summary>
...
> **Classical QKV ↔ Mathematical Framework — side by side:**
>
> In classical attention, you compute Q, K, V individually, then combine them:
>
> | Classical step | Shape | Mathematical Framework equivalent |
> |---|---|---|
> | `Q = X @ W_Q` | `[seq, d_head]` | — (absorbed into W_QK) |
> | `K = X @ W_K` | `[seq, d_head]` | — (absorbed into W_QK) |
> | `scores = Q @ K.T / √d_head` | `[seq, seq]` | `A^h = x W_QK^h x^T` |
> | `V = X @ W_V` | `[seq, d_head]` | — (absorbed into W_OV) |
> | `out = softmax(scores) @ V @ W_O` | `[seq, d_model]` | `x W_OV^h` (per source token) |
>
> The key insight is that you almost never need to inspect Q, K, and V individually for interpretability. The composite matrices W_OV and W_QK capture everything: W_OV tells you *what information moves* from source to destination; W_QK tells you *which positions attend to which*. The full circuits `W_E W_OV W_U` and `W_E W_QK W_E^T` then connect these directly to vocabulary tokens — mapping input tokens to output logits and input pairs to attention scores, respectively.
>
> **Heads-up on notation**: the Anthropic paper defines W_OV and W_QK using left-multiplication (row vectors appear to the left of matrices), so their W_OV corresponds to `W_V^T W_O^T` in their paper's convention. TransformerLens uses right-multiplication (`new = old @ weight`), so here `W_OV = W_V W_O`. The definitions above follow TransformerLens convention throughout.
>
> **From the paper — path expansion and the privileged basis (Elhage et al., 2021 §"Path Expansion" and §"Residual Stream"):**
>
> For a **two-layer attention-only model**, the paper expands the logits as a sum of interpretable **paths** through the network:
> $$\text{logits} = \underbrace{x_0 W_U}_{\text{direct}} + \sum_{h \in L_0} \underbrace{\sum_{s'} A^h_{s,s'} x_{s'} W_{OV}^h W_U}_{L_0 \text{ heads}} + \sum_{h \in L_1} \underbrace{\sum_{s'} A^h_{s,s'} x_{s'} W_{OV}^h W_U}_{L_1 \text{ heads (direct)}} + \underbrace{\text{K/Q/V-composition terms}}_{\text{cross-layer}}$$
> The composition terms arise from layer-1 heads whose attention patterns or value inputs are influenced by layer-0 outputs. Each path is a product of interpretable matrices, which is why the framework enables circuit-level analysis.
>
> **Privileged basis** (paper §"Privileged vs. Non-Privileged Bases"): $W_E$ and $W_U$ are in the **token basis** — they connect to vocabulary indices that have direct semantic meaning. The residual stream between layers has *no* privileged basis: you can insert any rotation $R$ (with $R R^T = I$) after every write and before every read without changing the model's behavior, because $R R^T = I$ cancels out. This is why the circuits that matter are end-to-end ones like $W_E W_{OV}^h W_U$ (token → logit) and $W_E W_{QK}^h W_E^T$ (token pair → attention score), not intermediate activations.
...
> **In plain language — why FactoredMatrix?**
>
> The full OV circuit `W_E @ W_V @ W_O @ W_U` is a `[d_vocab, d_vocab]` = `[50278, 50278]` matrix — about 2.5 billion floats. That doesn't fit in GPU memory and would be slow to compute. But this huge matrix is secretly a product of thin matrices: `(W_E @ W_V)` has shape `[50278, 64]` and `(W_O @ W_U)` has shape `[64, 50278]`. The actual "information content" lives in the small 64-dimensional (= `d_head`) bottleneck.
>
> `FactoredMatrix` stores only these two thin factors and derives properties — Frobenius norm, eigenvalues, singular value decomposition — from the small matrices without ever materializing the large product. Concretely:
> - **Frobenius norm**: equals √(sum of squared singular values), computable from the small SVD.
> - **Eigenvalues**: the non-zero eigenvalues of `AB` equal those of `BA`, which is a much smaller matrix.
> - **Copying score**: the positive eigenvalues of the OV circuit measure how strongly the head copies tokens to themselves — a key diagnostic for induction heads.
>
> **From the paper — copying score via eigenvalues (Elhage et al., 2021 §"Copying Matrix"):**
>
> The paper analyzes the **full OV circuit** $W_E W_{OV}^h W_U \in \mathbb{R}^{d_\text{vocab} \times d_\text{vocab}}$ to define a **copying score**. Diagonal entry $(t, t)$ measures how much attending to source token $t$ increases the logit for predicting token $t$ at the destination — i.e., how much the head "copies" that token to itself.
>
> Because $W_{OV}^h = W_V^h W_O^h$ has rank at most $d_\text{head}$, the paper uses the eigenvalue trick: the non-zero eigenvalues of $W_{OV}^h$ equal those of the smaller $d_\text{head} \times d_\text{head}$ matrix $W_O^h W_V^h$:
> $$\lambda_i(W_V^h W_O^h) = \lambda_i(W_O^h W_V^h), \quad i = 1, \ldots, d_\text{head}$$
> Eigenvalues near $+1$ indicate strong copying (attending to token $t$ → predict token $t$); eigenvalues near $-1$ indicate anti-copying; near $0$ means the head mainly moves information without preserving token identity.
>
> The Frobenius norm used throughout this section satisfies $\|M\|_F^2 = \sum_i \sigma_i(M)^2$ (sum of squared singular values), which equals $\sum_i \lambda_i(M)^2$ when $M$ is symmetric. For the composition scores, this makes $\|W_A\|_F$ and $\|W_B\|_F$ computable from small SVDs without materializing the full circuit.
...
<details><summary>Click to see the expected output (and interpretation)</summary>
...
> **In plain language — what composition scores measure:**
>
> Think of each attention head as occupying a small subspace of the 768-dimensional residual stream for its input and output. Two random heads in such a large space will almost certainly occupy *different* subspaces — their outputs and inputs barely interact. But if two heads are *deliberately composing*, the earlier head writes into exactly the subspace the later head reads from.
>
> The composition score `||W_A W_B||_F / (||W_A||_F * ||W_B||_F)` measures this alignment:
> - **High score**: W_A's output subspace aligns with W_B's input subspace → `W_A @ W_B` has large norm relative to each factor → heads are composing.
> - **Near-zero score**: W_A and W_B are orthogonal → `W_A @ W_B ≈ 0` regardless of individual sizes → heads are *not* composing.
>
> The three types of composition differ in *which* input wire of the later head (B) the earlier head (A) is feeding:
> - **Q-composition**: earlier head's output feeds the queries of the later head. W_B = W_Q (W_K)^T.
> - **K-composition**: earlier head's output feeds the keys. W_B = (W_Q (W_K)^T)^T = W_K W_Q^T.
> - **V-composition**: earlier head's output feeds the values. W_B = W_V W_O.
>
> For the induction circuit, we expect a high *K-composition* score between head 0.7 (prev-token head) and heads 1.4 / 1.10 (induction heads), because 1.4's key vectors are determined by 0.7's output.
>
> **From the paper — formal definition and virtual weights (Elhage et al., 2021 §"Residual Comms"):**
>
> The paper defines the composition score as:
> $$C(W_A, W_B) = \frac{\|W_A W_B\|_F}{\|W_A\|_F \cdot \|W_B\|_F}$$
> and specifies the $W_B$ matrix for each composition type:
>
> | Composition | $W_A$ (earlier head output) | $W_B$ (later head input) |
> |---|---|---|
> | Q-composition | $W_{OV}^{h_A}$ | $W_{QK}^{h_B} = W_Q^{h_B}(W_K^{h_B})^T$ |
> | K-composition | $W_{OV}^{h_A}$ | $(W_{QK}^{h_B})^T = W_K^{h_B}(W_Q^{h_B})^T$ |
> | V-composition | $W_{OV}^{h_A}$ | $W_{OV}^{h_B} = W_V^{h_B} W_O^{h_B}$ |
>
> **Why the Frobenius metric works**: for two random matrices in $\mathbb{R}^{d \times k}$ with $k \ll d$ (as in attention heads where $d = d_\text{model}$ and $k = d_\text{head}$), the expected composition score is approximately $1/\sqrt{d_\text{model}}$. A score well above this baseline indicates genuine composition — the two heads deliberately share a subspace. This follows from $\|M\|_F^2 = \sum_i \sigma_i(M)^2$ and the fact that random projections have nearly orthogonal column spaces in high dimensions.
>
> The paper calls the resulting cross-layer product a **virtual weight** or **virtual attention head**: e.g., K-composition between $h_A$ and $h_B$ creates the virtual QK circuit $W_{OV}^{h_A} W_{QK}^{h_B}$, which acts as if there were a single head whose attention pattern is determined by the composition. The full token-level virtual circuit $W_E W_{OV}^{h_A} W_{QK}^{h_B} W_E^T$ is exactly the K-composition matrix from the induction circuit analysis.
